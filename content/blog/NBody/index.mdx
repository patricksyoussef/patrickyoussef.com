...
tags: ["Simulation", "Python", "NBody", "Orbital Mechanics"]
excerpt: "TBD"
pinned: true
---
...
This post will introduce you to the principles behind predicting the motion of gravitationally interacting objects. Using Python with some popular modules, we'll implement the principles from scratch and look at some fun examples such as the one one below. Overall expect this to be more practical coding and testing and less theory as much of the core numerical theory has been covered in a [prior post](/blog/integrator-intro).

<PostVideo video={"/videos/NBody/SolarAnim.mp4"} _autoplay={true} _muted={true}/>
...
Like with many problems, there are varieties of methodologies behind how someone may solve it. When developing this solution I focused on a few principles that are good to keep in mind when following along.
...
- **Efficient**: This problem naturally scales quadratically so it's important to produce efficient code for the core operations. 
...
I'll summarize some of the notation and shorthands I use. This includes my vector notation, variable assignments, and some others that might seem foreign but should be explained through reading the post.
...
# Review of Gravity Laws and Associated Equations

In this section, I will give a brief overview of Newton's law of universal gravitation and extend the concept of gravitation between two bodies to all of our $n$ bodies.
...
In plain english, the net force on a particular body is the sum of the force equation evaluated between the body of interest and all *other* bodies. The other comes into play as the $j \neq i$ where we do not calculate the attraction of a body onto itself as a body cannot attract itself and mathematically we would be dividing by 0 when calculating the force.

## Net Acceleration on One Body
...
We can actually use this to summarize the n-body problem as a systems of equations and provide the differential equation that we intend to integrate in a later section.
...
## Net Acceleration for All Bodies
...
In this method, we will try to leverage the property that each force we compute can be negated to provide a second force that we can use. In that case, we can resolve two force in our sets of forces that we need to calculate and reduce our force calculations by half.

$$
K = \frac{n(n-1)}{2}
$$

In practice, all this entails is computing the forces for all combinations of bodies! Looking at the equation for number of combinations we can see that this reduces to the prior assertion for operation count. To be precise, we'll be looking at all combinations of size 2 ($r=2$) from the set of all our bodies. I call the set of combinations the pairs of bodies ($P$) throughout the rest of the post.
...
For those who are familiar with $O(n)$ notation you might say well aren't these the same runtime as we ignore constants? In terms of the general growth of the function with $n$, yes both are these methods are still quadratic in nature but if you were to measure the actual runtime we would see a gain. This is an example where trying to compare time complexity to runtime can get messy.
...
Kinetic is really easy! We just need to sum up the result a classic physics equation.
...
So I have been throwing around the word state a bit loosely, let's try to refine our understanding of what this means and why it's useful for us. As my friends ask me to do often, the 30 second answer is that a state is a matrix that maintains all of the important values that describe the aspects of our system such as the positions and velocities of all of our objects.

> *TLDR: States are a joint representation of everything going on in our system. They help us simplify our integration operations and keep track of our variables.*
...
The prior would be the state for a 2D case with two objects, don't confuse $X$ with $x$, I maintain that the capital $X$ is the state and has nothing to do with the position variable $x$.
...
For simple systems, it might be easier to just build out a numerical integrator for each of the variables of interest, but for large n-body systems this would get messy quickly. Consider an example in 3D with 50 bodies, this would mean that we have 300 variables of interest and therefore 300 integrators! When we represent this system in the state matrix form, there are still the 300 values being updated but it's reduced to a series of sums between the entire matrices which is much cleaner.
...
# Refresher on Integrators

Although it helps to have read my [post](/blog/integrator-intro/) on integrators already, I'll do a short summary of the usage of the RK4 integrator that we are using in this post. If you are already comfortable doing the example cases from that posts you can skip ahead.
...
Let's go ahead and write our differential equation into a function that will return the state derivative for a given state.
...
The prior formulations maps really well into the code! Also there are faster ways to compute these by using vectorization and maintaining matrices as opposed to iterating but this is way easier to understand which is why I opted to present it this way.
...
Generally speaking, the RK4 integrator is a good all-around integrator for various types of problems (to me it's reliable but likely not optimal depending on the case). As an example, simulating Hamiltonian systems (often called conservative system) one can use special integrators that preserve the constant energy nature of these systems. I won't get into it here, but I'm just trying to imply that there isn't one *best* integrator.
...
Before we step into larger and more interesting examples, it helps to validate that the work that we did is providing results that we think are correct. Let's consider a simple case of a small satellite (something akin to a Starlink satellite) orbiting in a circular low earth orbit (LEO).
...
In this section, we'll have fun with an N-body choreography with three bodies. The history of choreographies is [quite short](https://www.math.uni-bielefeld.de/~rehmann/ECM/cdrom/3ecm/pdfs/pant3/simo.pdf) and many fun choreographies hav already been found. We're focusing on one that should produce a stable figure 8!

> *It's pretty typical for these choreographies to use $G=1$*
...
<PostVideo video={"/videos/NBody/3BodyAnim.mp4"}/>

# Partial Solar System

Lastly, let's consider a partial solar system example. I limited this to partial as the periods of orbit for some of the outermost planets is quite long and I don't think it's reasonable for most people to run a full period of these orbits on their computers.
