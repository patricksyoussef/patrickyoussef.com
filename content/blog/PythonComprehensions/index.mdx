...
- You can transform some iterative code into a more readable one-liner
- Often more time and space efficient that the iterative (for loop) variant
...
For smaller tasks like this this structure is fine but as we add other bits of utility it can become harder to read the code while also adding additional overhead. Let's try to replicate this structure as a comprehension.

## List Creation via Comprehension

For a list comprehension, we wrap a statement that does what our prior for loop does within brackets to produce a resultant list that is equivalent to the list from before.

![Structure Of a List Comprehension](./images/ListBase.png)

The list comprehension handles the list initialization and appending that we did before and reduces the code to just the important bits, what are we iterating over and what are we doing to each of the elements.
...
One trend you may be noticing is that everything after the expression in the comprehension is just writing in the exact part of the loop! if you go over the iteration and condition line in the for loop example you'll see writing those in exactly as is into the comprehension works!
...
If you read other resources you might also see nested comprehensions where you can put a comprehension into another list comprehension and generate a matrix or iterate over a matrix. To me, these kind of step away from the core principles of readability and simplicity that make comprehensions so great. I would recommend away from complicated comprehensions and expanding that code if the comprehension becomes unreadable.
...
You may not believe me, but you already grasp most of the information to understand the unfamiliar sounding generator comprehensions! Generator comprehensions are extremely similar to list comprehensions with some quirks that can be very useful.

## What's a Generator?

> TLDR: Generators don't store all the results but just pass them one at a time when you call the generator which is great for saving memory but not great if you want to save the resultant list.
...
In my opinion, generator comprehensions are the hardest to understand in practice so I think the hardest is over! Set comprehensions are ones that I often use and luckily they're super simple as well!

## Set Comprehensions as List Comprehensions

In practice, a set comprehension can be thought of as a list comprehension but the list is converted to a set after. As a reminder, a set is an unordered collection of items where no items can repeat.
...
It's about what you would expect at this point where we so some sort of initialization, get items from out iterable, and run some code to define changes to our data structure.
...
In this example, we'll do a similar case as before where the key is the input number and the value is the floored value while keeping the positive value only condition.
